<?php
	...
	//include config
	include "config.php";
	$titleMsg="Tender-Forms-Negotiated-Detail-Report";
	$reportLot="";
	$reportSeason="";
	...
	<!-- content starts here -->		    	
        ...
        <!-- post starts here -->				            
            <div class="post">
                ...
                <br />
                	<form action="report-tender-negotiated-forms.php" method="post" name="tenderReportForm" id="tenderReportForm"> 
			     	...
			     	if($action=="report")
			     	{
			     		...
			     		if(isset($reportData))
			     		{
				            ...
				            echo("<h2>Forest Working Division ".$_SESSION['division']." </h2>");
				     		echo("<h1>Negotiated Tender Results for Resin Lots ".$_POST['lot_no']." for Season ".$_POST['season_year']."</h1>");
				     		
				     		echo("<table class='reportTable'> <tr class='headRow'> <td>Sr. No.</td> <td>Lot No.</td> <td>No of Blazes</td> <td>Yield per Section</td>  <td>Total Turnout</td> <td>Tenderer Name and Address</td>  <td>Class</td>  <td>Rate Offred</td> <td>Rate Negotiated</td> <td>Amount</td>  <td>Rate Fixed</td> <td>Amount</td>  <td>Variation</td> <td>%age</td> <td>EMD</td>  <td>Remarks</td> </tr>");
				     		$index=0;
				     		foreach ($reportData as $data )
			            	{
			            		$record = $db->get_row("SELECT id, division_code, unit_code, lot_no, blazes_received, proposed_yield, approved_yield, approval_yield_status, total_turnout, total_expenditure, rate_calculated, proposed_rate, approved_rate, approval_rate_status, season_year FROM t_proposed_price_for_lot WHERE lot_no='".$data['lot_no']."' AND season_year='".$data['season_year']."'",ARRAY_A); 
				     			$index++;
				     			$rateOffered=$data['negotiated_rate'];
				     			if($rateOffered<=0)
				     			{
				     				$rateOffered=$data['rate_offered'];
				     			}
				     			$offeredAmount=round($rateOffered*$data['total_turnout'],2);
				     			$rateApproved=$record['approved_rate'];
				     			$approvedAmount=round($record['approved_rate']*$data['total_turnout'],2);
				     			$variationAmount=round(($offeredAmount-$approvedAmount),2);
				     			$percentage= round((($variationAmount/$approvedAmount)*100),2);
			            		...
			            		"</td><td>".$data['contractor_class']."</td> <td>".$data['rate_offered']."</td><td>".$data['negotiated_rate']."</td><td>".$offeredAmount."</td> <td>".$rateApproved."</td><td>".$approvedAmount."</td> <td>".$variationAmount."</td><td>".$percentage."</td> <td>".$data['em_deposited']." ".$data['em_mode']."</td> <td>rem</td></tr>");
			            	}
