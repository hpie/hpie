...
if($totalRecords > 0){
	...
	$paging->startRow	=	$recNo;
	$paging->setPageNo();
	$paging->query = "SELECT DISTINCT t1.i_forest_id ,t1.id as eid,t1.i_timber_id,t1.price,t1.i_status,t2.id,t2.vc_name as f_name FROM m_timber_price as t1 join m_forest as t2 where t1.i_forest_id=t2.id group by t1.i_forest_id ORDER BY $paging->orderBy limit $paging->startRow,$paging->recsPerPage";
