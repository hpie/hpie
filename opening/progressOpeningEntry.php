...
if($updatePrevious > 0)
	...
	$list=$common->selectCondition('opening_progress_felling_master', $arFieldValues);
	echo count($list);
	...
	else
	{
	    ...
	    $progressId = $db->insert('opening_progress_felling_master',$arMarkedPrice);
		}
		else
		{
		...
		foreach($catvalue as $key=>$categoryValueArray){
			...
			foreach($categoryValueArray as $id=>$value){
				...
				$id = $db->insert('opening_progress_felling_detail',$fellingDetail);
			}
		}
		...
		foreach ($fellingCharges as $key=>$categoryValueArray)
		{
			...
			$chargesId = $db->insert('opening_progress_felling_charges',$fellingChargesArray);
		 ...
		 $id = $db->update('opening_progress_felling_master',$arFieldValues,$arCondition);
	  

	}
	ob_end_clean();
	header("Location:".BASE_URL."index.php?page=fellingHomeOpening&markId=".$markDetailId);
...
$progressMasterDetail=$common->selectCondition('opening_progress_felling_master', $arFieldValues);
...
$voulmeDetail=$markDetail->getFellingConversionDetailOpening($progressID);
	...
	for(y=0;y<treesArray.length;y++){
	    for(x=0;x<catArray.length;x++){
           ...
           exisVolume=parseFloat(exisVolume)+parseFloat(catVolume)* parseFloat(catValue);
       	   document.getElementById('catVolume['+catArray[x]+']').value=roundNumber(exisVolume,2);
      }
    }
...
<table align="center" width="90%" border="0" cellspacing="0"
	...
	<tr>
		<td colspan="2" align="center"><b><u>Felling Opening</u></b> <br></br>
		</td>
	</tr>
	...
	<tr>
		...
		?>
			<tr>
				<td width="30%">
				<input name='i_month_id' type='hidden' value='1' />
		...
		<table>
			<tr>
				...
				<table border=0 style="width: 20%" cellspacing="0" cellpadding="0">
					...
					onclick="javascript:parent.location='index.php?page=fellingHomeOpening&markId=<?php echo $markDetailId;?>'" /></td>
...
function outFocus(control,tree)
{
