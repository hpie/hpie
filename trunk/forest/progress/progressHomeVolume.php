...
if($updatePrevious > 0)
	...
	$list=$common->selectCondition('progress_conversion_actual_master', $arFieldValues);
	echo count($list);
	...
	else
	{
	    ...
	    $progressId = $db->insert('progress_conversion_actual_master',$arMarkedPrice);
		}
		else
		{
		...
		$db->deleteProgressFellingDetailVolume($progressId);
		$db->deleteProgressFellingChargesVolume($progressId);
		$totalCount=0;
		$totalVolume=0;
		foreach($catvalue as $key=>$categoryValueArray){
			...
			foreach($categoryValueArray as $id=>$value){
				...
				$fellingDetail['i_volume']=($prevcatvolume[$key][$id]);
				$fellingDetail['i_felling_charges']=($fellingCharges[$key]*$volumeTableDetail[$key][$id]['volume']*$value);
				
				$totalCount+=$value;
				$totalVolume+=($prevcatvolume[$key][$id]*$value);
				$id = $db->insert('progress_conversion_actual_detail',$fellingDetail);
			}
		}
		...
		foreach ($fellingCharges as $key=>$categoryValueArray)
		{
		 ...
		 $id = $db->update('progress_conversion_actual_master',$arFieldValues,$arCondition);
	  

	}
	ob_end_clean();
	header("Location:".BASE_URL."index.php?page=conversionHomeStep1&markId=".$markDetailId);
...
$progressMasterDetail=$common->selectCondition('progress_conversion_actual_master', $arFieldValues);
...
$voulmeDetail=$markDetail->getFellingConversionDetailVolume($progressID);

$tableP= 'progress_conversion_actual_detail';
...
$markList=$markDetail->getFeelingDetailSummarry($markDetailId);

$voulmePrevDetail=$markDetail->getConversionPreviousConversionDetail($markDetailId,$progressID);
$arrAllcontractors=$common->getContractorWork($markDetailId,2);
	    ...
	    for(x=0;x<catArray.length;x++){
           ...
           var catVolume=0;
             //try 
             {
             	 ...
             	 catVolume=document.getElementById("prevcatvolume["+treesArray[y]+"]["+catArray[x]+"]").value;
                 if(catVolume ==100) alert(catVolume);
                  }
             //catch(error)
             {
...
<ul id="menu">
	<li><a id="<?php echo pageUrl("home",$currentPage)?>"
		href="index.php?page=conversionHomeStep1&markId=<?php echo $markDetailId;?>" title="Goto Home Page">Back</a></li>
</ul>
<br>
...
<table align="center" width="90%" border="0" cellspacing="0"
			...
			<td colspan="3" align="left">
				<table border=0>
					...
					if($progressID > 0)
					{
						?>
					<tr>
						<td width="30%"><?php echo $arrMonths[$i_month_id];?>/<?php echo $arrYears[$i_year];?>
						</td>
						<td><input name='i_month_id' type='hidden'
							value='<?php echo $i_month_id;?>' /> <input name='i_year'
							type='hidden' value='<?php echo $i_year;?>' />
						</td>
					</tr>
					<?php	
					}
					else
					{
						?>
					<tr>
						<td width="30%"><?php echo makeSelectOptions($arrMonths,"i_month_id",$i_month_id,"",150,"class='required'");?>
						</td>
						<td><?php echo makeSelectOptions($arrYears,"i_year",$i_year,"",70,"class='required'");?>
						</td>
					</tr>
					<?php 
		  }
		...
		foreach($catList as $key=>$category){
			...
			<td style="width: 20%">
				<table>
					...
					<tr>
						<td><input type='text'
							onblur="outFocusCat(this,'<?php echo $volumeTableDetail[$category->id]['volume']?>','<?php echo $category->id;?>','<?php echo $k;?>','prevcatvalue[<?php echo $k;?>][<?php echo $category->id;?>]')"
						    onfocus="getFocus(this);" class="required number"
							name="prevcatvolume[<?php echo $k;?>][<?php echo $category->id;?>]"
							id="prevcatvolume[<?php echo $k;?>][<?php echo  $category->id;?>]"
							value="<?php echo (($voulmeDetail[$k][$category->id]->i_volume =='' || $voulmeDetail[$k][$category->id]->i_volume <= 0  )? $volumeTableDetail[$k][$category->id]['volume'] :$voulmeDetail[$k][$category->id]->i_volume );?>"
							style="width: 50px;" />
	...
	<tr>
		...
		<input style=" width:50px;" class="required number"
    ...
    <tr>
		...
		<td style="width: 20%"><input class="required number"
			...
			value="<?php   echo($fellingCharges[$category->id]['i_felling_charges']); ?>"
			style="width: 50px;"  /></td>
			<?
		}
		?>
		...
		<table>
			<tr>
				...
				<table border=0 style="width: 20%" cellspacing="0" cellpadding="0">
					...
					onclick="javascript:parent.location='index.php?page=conversionHomeStep1&markId=<?php echo $markDetailId;?>'" /></td>
...
function outFocusCat(control,volume,category,tree,preValue)
{
