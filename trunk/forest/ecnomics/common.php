...
function getExtraCharges($mark_id){
	...
	function getAllLotDetail($lotNo=''){
		global $db;
...
function checkExistingPriceOption($table,$id,$fId){
	...
	function updateEcmonicsDetails( $markId,$ecnomicsId)
	{
		...
		$sql  = "update r_tree_category a, ecnomics_category_conversion b  set a.i_ecnomics_pesentage =b.i_value
		         where a.i_category_id =b.i_category_id	 and b.i_mark_id =a.i_mark_detail_id  
	   			 and b.i_mark_id='".$markId."' and b.i_ecnomics_master_id='".$ecnomicsId."'";
		...
		$sql  = "update c_marked_volume a, ecnomics_category_conversion b ,ecnomics_category_felling c set a.i_economics_conversion =b.i_value ,i_economics_volume=i_tree_volume*(b.i_value/100)
				 ,a.i_felling_charges=c.i_felling_charges ,a.i_conversion_charges=c.i_conversion_charges
				where a.i_category_id =b.i_category_id	 and b.i_mark_id =a.i_mark_id
	   			and b.i_mark_id =c.i_mark_id and c.i_category_id =b.i_category_id and  b.i_mark_id='".$markId."'  and b.i_ecnomics_master_id='".$ecnomicsId."' and  c.i_ecnomics_master_id='".$ecnomicsId."'";
		$hRes = mysql_query($sql);
	...
	function checkExistingEcnomics($i_mark_id){
		global $db;
		$sql = "SELECT * FROM economics_master WHERE i_mark_id = '".$i_mark_id."' AND id in (select max(id) FROM economics_master WHERE i_mark_id = '".$i_mark_id."' )";
		$row = $db->select($sql);
		if(!empty($row)){
			return $row[0];
		}else{
...
function getEcnomicsDateSelect($i_mark_id)
	...
	name='markid' />
	<input
	type='hidden' value='<?php echo $markDetailId?>' id='onlyTree'
	name='onlyTree' />
	<center>
	<table style="width: 250px">
	<tr>
	...
	function updateProductDetails( $markId,$ecnomicsId,$timber_id)
	{
		...
		$sql  = "update c_marked_volume a, ecnomics_category_timber_temp  b  set a.i_product_persentage =b.i_value 
				  where a.i_category_id =b.i_category_id and b.i_mark_id =a.i_mark_id
	   			  and  b.i_timber_id='".$timber_id."'  and b.i_ecnomics_master_id='".$ecnomicsId."'";
...
function getForestExitPoints($markdId,$treeId){
	...
	function getForestEntryPointsRSD($markdId,$treeId,$vc_month,$vc_year,$i_timber_id,$i_timbertype_id){
		...
		$arrContractors = array();
		$sql  = "SELECT * FROM m_forest_points WHERE i_status = '1'
		and i_type  in (1,2) and i_forest_id  in  (select i_forest_id  from c_mark_detail where id ='".$markdId."')
		and id  in (
		select vc_destination from progress_transportation_detail
		where    i_exit  = 1 and i_tree_id='".$treeId."' and i_progress_id in ( select id from progress_transportation_master where
		i_mark_id  ='".$markdId."'
	...
	function generateSelectForTimber($markId,$treeId)
	{
		...
		$sql  = "SELECT distinct i_timber_id,b.vc_name as timberName	 FROM
		 progress_conversion_detail a,
         m_timber b
		  WHERE
		  a.i_timber_id=b.id and 
		i_progress_id  in
		(select id  from progress_conversion_master
		where i_mark_id  ='".$markId."')
	...
	function getContractorDetail($id){
		...
		if(!empty($rows)){
			foreach($rows as $row){
	            ?>
	            <table>
	             <tr class='odd'> <td>Name</td> <td><?php echo $row['contractor_name'] ?></td></tr>
	             <tr class='even'> <td>Phone</td> <td><?php echo $row['phone'] ?></td></tr>
	             <tr class='odd'> <td>Licence-No</td><td><?php echo $row['license'] ?> </td></tr>
	             <tr class='even'> <td>Class</td> <td><?php echo $row['contractor_class'] ?></td></tr>
	             </table>
	            <?php 
			}
		}
