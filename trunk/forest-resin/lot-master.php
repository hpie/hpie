<?php
	...
	if(isset($_POST['submitted']))
	{
		...
		{
			$db->query("INSERT INTO m_lot (id, lot_no, lot_desc, forest_code, division_code, created_by) VALUES (NULL, '".$_POST['lot_no']."', '".$_POST['lot_desc']."', '".$_POST['forest_code']."', '".$_SESSION['division']."', '".$_POST['created_by']."'");
			$db->debug();
			if($db->rows_affected>0)
			{ 
            ...
            <div class="post">
              	...
              	?>
              		<form action="lot-master.php" method="post" id="lotForm" data-validate="parsley">
						...
						<legend><font size=5 color=#72A545>Update Lot</font></legend><br />
							...
							<div style="margin:10px; 0px; 0px; 0px;">
								<label for="lot_no">Lot No:</label>
								<input class="textbox" id="lot_no" type="text" name="lot_no" data-required="true" data-error-message="Lot Number is required" value="<?php echo($lot['lot_no']);?>"/>
								<label for="lot_desc">Lot Description:</label>
				...
				<?php 
			  		}else
                	{
                		...
                		$lots = $db->get_results("SELECT * FROM m_lot GROUP BY lot_no",ARRAY_A);
